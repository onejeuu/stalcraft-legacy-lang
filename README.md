...
Если вам надоело или не понравилось, то модификацию можно удалить ещё раз запустив скрипт. Он обнаружит резервную копию и восстановит файлы локализации.

Это сработает, если за время использования в игре не выходило патчей.

Резервные копии хранятся рядом с оригиналом, имея `.bck` в конце названия.
Файлы локализации находятся по пути `assets/gloomycore/lang` и `assets/stalker/lang`.
